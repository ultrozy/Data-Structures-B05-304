...
void Stack<T>::PushBack(const T& value) {
  ++size_;
  back_ = new NodeStack{back_, value};
}
...
Stack<T>::Stack(const Stack<T>& other) : back_{nullptr}, size_{other.size_} {
  ...
  NodeStack* p_node = back_ = new NodeStack{nullptr, other.back_->value};
  NodeStack* p_other_node = other.back_;
  while (p_other_node->next) {
    p_other_node = p_other_node->next;
    p_node = p_node->next = new NodeStack{nullptr, p_other_node->value};
  }
}
